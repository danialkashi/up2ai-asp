...
PW = os.environ.get("PREVIEW_ADMIN_PASSWORD", "")  # رمز پنلِ همان سروری که ازش عکس می‌گیریم
DEMO_USER = "admin"
...
def get(path: str) -> str:
    with opener.open(BASE + urllib.parse.quote(path, safe="/?=&#%"), timeout=30) as r:
        return r.read().decode("utf-8")


def get_allow_error(path: str) -> str:
    """مثل get، ولی کدهای ۴۰۴/۵۰۰ را هم به‌جای استثنا برمی‌گرداند."""
    try:
        return get(path)
...
def login() -> None:
    if not PW:
        sys.exit("رمز پنل داده نشده. متغیر PREVIEW_ADMIN_PASSWORD را ست کن.")
    ...
    # کوکی نشستِ پنل در محیط Production پرچم Secure دارد (که درست است). این
    # اسکریپت اما با http ساده به سرورِ محلی وصل می‌شود و کتابخانه‌ی کوکیِ
    # پایتون کوکی Secure را روی http نمی‌فرستد — نتیجه این می‌شد که همه‌ی
    # صفحه‌های پنل به‌جای محتوای واقعی، «صفحه‌ی ورود» را برگردانند و
    # پیش‌نمایش بی‌صدا خالی از آب دربیاید. پرچم را فقط در همین حافظه‌ی موقت
    # برمی‌داریم؛ نه سروری عوض می‌شود نه فایلی نوشته.
    for cookie in jar:
        cookie.secure = False

    if "محتوای سایت" not in get("/admin"):
        sys.exit("ورود به پنل انجام نشد — رمزِ PREVIEW_ADMIN_PASSWORD را بررسی کن.")
...
not_found = body_of(get_allow_error("/این-صفحه-وجود-ندارد"))
# صفحه‌های عمومیِ وبلاگ *قبل* از ورود گرفته می‌شوند تا دقیقاً همان چیزی
# باشند که یک بازدیدکننده‌ی معمولی می‌بیند.
blog_list = body_of(get("/blog"))
blog_post = ""
# نوشته‌ای که نظرِ تأییدشده دارد بهترین نمونه برای پیش‌نمایش است، چون هم متن
# را نشان می‌دهد و هم بخش نظرها را. اگر چنین نوشته‌ای نبود، اولی برداشته
# می‌شود.
cards = re.findall(r'<a href="(/blog/[^"?#]+)"[^>]*>(.*?)</a>', blog_list, re.S)
pick = next((href for href, card in cards if "نظر</span>" in card or "نظر<" in card), None)
pick = pick or (cards[0][0] if cards else None)
if pick:
    blog_post = body_of(get(urllib.parse.unquote(pick)))

login()

# اولین نوشته برای نمای «ویرایشگر نوشته»؛ اگر وبلاگ خالی باشد، فرمِ خالی.
admin_posts = get("/admin/posts")
edit_id = re.search(r'/admin/posts/edit\?id=([0-9a-f-]+)', admin_posts)
post_editor = get("/admin/posts/edit?id=" + edit_id.group(1)) if edit_id else get("/admin/posts/edit")

views = {
    "site": ("صفحه‌ی اصلی سایت", body_of(home)),
    "site-blog": ("سایت — فهرست وبلاگ", blog_list),
}
if blog_post:
    views["site-post"] = ("سایت — یک نوشته و نظرها", blog_post)
views.update({
    "site-404": ("سایت — صفحه‌ی ۴۰۴", not_found),
    ...
    "admin-users": ("پنل — کاربران", body_of(get("/admin/users"))),
...
#pv-bar span.sep { width:1px; align-self:stretch; background:#374151; margin:0 4px }
...
bar = "".join(
    # جداکننده بین نماهای سایت و نماهای پنل، چون فهرست حالا بلند است
    ('<span class="sep"></span>' if k == "admin-login" else "")
...
out = sys.argv[2] if len(sys.argv) > 2 else "UP2AI-preview.html"
